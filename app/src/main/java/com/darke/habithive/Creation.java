...
//        Toast.makeText(getContext(), String.format("Email: %s, Name: %s", email, name), Toast.LENGTH_SHORT).show();
        ...
        habitNameInput = findViewById(R.id.input_habit_name);
...
//      Daily weekly Monthly Button
        ...
        dailyDynamicContentContainer = findViewById(R.id.daily_dynamic_content_container);
        weeklyDynamicContentContainer = findViewById(R.id.weekly_dynamic_content_container);
        monthlyDynamicContentContainer = findViewById(R.id.monthly_dynamic_content_container);
...
//       Goal View
        ...
        radioTimer = findViewById(R.id.radio_timer);
        radioChecklist = findViewById(R.id.radio_checklist);
        goalResponseContainer = findViewById(R.id.goal_response_container);
...
//       Goal View
        ...
        FloatingActionButton submitButton = findViewById(R.id.habit_submit_button);
    ...
    private void addRadioButtonsWithIcons() {
        for (int iconResId : iconResIds) {
            ...
            Drawable topDrawable = ContextCompat.getDrawable(this, iconResId);
            if (topDrawable != null) {
    ...
    private void updateCheckboxColor(@NonNull CheckBox checkBox) {
        ColorStateList colorStateList = ContextCompat.getColorStateList(this, R.color.colorChecked);
...
//        Type
        if (habitTypeRadioGroup.getCheckedRadioButtonId() == -1) {
            Toast.makeText(this, "Please select a habit type", Toast.LENGTH_SHORT).show();
            return;
        }
...
//        Frequency
        ...
        if (frequency.equals("Daily")) {
            ...
            if (daysOfWeek.stream().noneMatch(day -> day == 1)) {
                Toast.makeText(this, "Please select at least one day of the week",
            ...
            if (dayOfMonth.isEmpty()) {
                Toast.makeText(this, "Please select at least one day of the month",
                        Toast.LENGTH_SHORT).show();
                return;
            }
...
//        Goal
        ...
        if (goalType.isEmpty()) {
            Toast.makeText(this, "Please add a goal response",
                    Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (goalResponseContainer.getChildCount() == 0) {
            Toast.makeText(this, "Please add at least one goal response",
                    Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        db.collection("users").document(user.getUid()).collection("habits")
                ...
                .addOnFailureListener(e -> Toast.makeText(this, "Error saving habit", Toast.LENGTH_SHORT).show());
